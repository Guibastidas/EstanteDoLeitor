...
async function loadSeriesDetail(seriesId) {
    try {
        ...
        // Calcular progresso considerando APENAS as edições LIDAS (independente do número)
        ...
        // Progresso com nota sobre exceção
...
function displayIssues(issues, totalBaixado, totalPublicado) {
    ...
    for (let numero = 1; numero <= totalPublicado; numero++) {
        ...
        // ✅ NOVA LÓGICA:
        // - Edição antiga E NÃO lida E existe no sistema = VERMELHO (pode marcar como lida)
        // - Edição antiga E NÃO lida E NÃO existe = VERMELHO (não baixada)
        // - Edição antiga E LIDA = VERDE (conta normalmente!)
        // - Edição normal = funciona como sempre
        ...
        if (issue) {
            ...
            if (issue.is_read) {
                // ✅ LIDA = VERDE (independente do número)
                colorClass = 'read';
                ...
                // ⚠️ Edição antiga NÃO lida = VERMELHO (mas pode marcar como lida!)
                ...
                // ⚪ Edição normal baixada mas não lida
                colorClass = '';
                titleText = `Edição #${numero}`;
                actionsHTML = `
                    <label class="checkbox-icon" title="Marcar como lida">
...
async function sincronizarEdicoesAutomaticamente() {
    ...
    try {
        ...
        // Adicionar todas as edições faltantes
        let adicionadas = 0;
...
/**
 * Verificar se o contador "Lendo" está sincronizado com as edições realmente marcadas como lidas
 */
async function verificarSincronizacaoLendo() {
    ...
    try {
        ...
        if (readIssuesReal === readIssuesRegistrado) {
            ...
            // Poderia adicionar correção automática aqui no futuro
        }
...
async function submitIssueForm(e) {
    ...
    try {
        ...
        // Atualizar total_issues se necessário
        const series = await fetchAPI(`/series/${currentSeriesId}`);
